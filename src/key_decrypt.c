...
void patch_sceMesgLed()
{
    ...
    for (u32 addr = text_addr; addr<topaddr; addr+=4){
        u32 data = _lw(addr);
        if (data == 0x2CE30001){
            mesgled_decrypt = (void*)addr; // Save Original Decrypt Function Pointer
            break;
        }
    }
    
    u32 func1 = sctrlHENFindFunction("sceMesgLed", "sceMesgLed_driver", 0x792A6126);
    u32 func2 = sctrlHENFindFunction("sceMesgLed", "sceMesgLed_driver", 0x337D0DD3);
